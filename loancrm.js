/*******************************
 * loancrm.js
 * Supabase-backed Bank Loan CRM (DOM-safe init)
 *******************************/
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* view toggles */
  window.showForm = function(editing = false) {
  ...
  window.showList = function() {
    ...
    fetchLoanClients();
    window.scrollTo({ top: listCard.offsetTop - 20, behavior: "smooth" });
  };

  /* create public url helper */
  ...
  async function fetchLoanClients(query = "") {
    ...
    items.forEach(row => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button onclick="onEditLoan(${row.id})" class="btn-plain">Edit</button>
          <button onclick="onDeleteLoan(${row.id})" style="margin-left:6px" class="btn-plain">Delete</button>
        </td>
      `;
  ...
  /* edit/delete handlers (exposed to global like before) */
