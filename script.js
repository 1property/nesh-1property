/***********************************************
 * FINAL script.js – STABLE + RENT ATTACHMENT FIX
 ***********************************************/
...
async function fetchRentData() {
  ...
  data.forEach(r => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ${r.attachment_url 
          ? `<a href="${r.attachment_url}" target="_blank">View</a>` 
          : "-"}
      </td>
      <td>
...
$("rentForm").addEventListener("submit", async e => {
  ...
  if (fileInput.files.length > 0) {
    ...
    const filePath = `rent/${Date.now()}_${file.name}`;
...
document.addEventListener("click", e => {
  ...
  if (e.target.classList.contains("btn-delete-rent")) {
    deleteContext = { table: RENT_TABLE, id: e.target.dataset.id };
    confirmDeleteModal.show();
  }
});
